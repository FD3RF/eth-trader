...
def fetch_with_retry(func, retries=RETRIES, base_delay=BASE_DELAY):
    for i in range(retries):
        try:
            ...
            wait = base_delay * (2 ** i)
            if i < retries - 1:
                st.warning(f"请求失败，{wait:.1f}秒后重试 ({i+1}/{retries})：{e}")
                time.sleep(wait)
            else:
                st.error(f"请求失败，已达最大重试次数：{e}")
...
def fetch_klines():
    ...
    if result and result.get('Response') == 'Success' and result.get('Data', {}).get('Data'):
        ...
        candles = [[item['time']*1000, item['open'], item['high'], item['low'], item['close'], item['volumefrom']] for item in data]
...
def get_higher_trend():
    ...
    result = fetch_with_retry(_fetch)
    if result and result.get('Response') == 'Success' and result.get('Data', {}).get('Data'):
        closes = [item['close'] for item in result['Data']['Data']]
        if len(closes) >= 200:
...
def volume_surge(df, vol_period=20, surge_mult=1.3):
    ...
    return (current_vol / avg_vol > surge_mult) and (df['close'].iloc[-1] > df['open'].iloc[-1])
...
def detect_signal_pro(df, fast, slow, rsi_period, buy_min, buy_max, sell_min, sell_max,
    ...
    if len(df) >= range_lookback:
        ...
        if ema_f_now > ema_s_now and last['close'] <= prev_high:
            return None
        if ema_f_now < ema_s_now and last['close'] >= prev_low:
            return None

    # 趋势扩散
    ema_spread_now = abs(ema_f_now - ema_s_now)
    ema_spread_prev = abs(df['ema_fast'].iloc[-2] - df['ema_slow'].iloc[-2]) if len(df) >= 2 else 0
    if ema_spread_now <= ema_spread_prev or ema_spread_now / last['close'] < 0.001:
        return None

    # 禁止刚交叉
    if len(df) >= 4:
        if (df['ema_fast'].iloc[-4] < df['ema_slow'].iloc[-4] and ema_f_now > ema_s_now) or \
           (df['ema_fast'].iloc[-4] > df['ema_slow'].iloc[-4] and ema_f_now < ema_s_now):
            return None
    ...
    if use_slope_filter and abs(slope_strength) <= slope_threshold:
        return None
...
def fill_missing_candles(buffer, new_candle):
    ...
    if new_ts > expected_ts:
        ...
        ts = expected_ts
        while ts < new_ts:
            missing.append([ts, buffer[-1][4], buffer[-1][4], buffer[-1][4], buffer[-1][4], 0])
...
def update_signal_stats():
    ...
    st.session_state.signal_stats = {'total': total, 'win': win, 'loss': loss, 'exit': exit_count, 'pending': pending, 'win_rate': win_rate}
...
def update_signal_result(index, result, exit_price=None, exit_reason='', note=''):
    ...
    if 0 <= index < len(history):
        ...
        if note:
            record['note'] = note
        update_signal_in_db(record['id'], result, record.get('exit_price'), record.get('exit_time'), exit_reason, record.get('peak'), record.get('note'))
...
<style>
    * {font-family: 'Inter', 'Microsoft YaHei', sans-serif;}
    ...
    .tp2-container {background:linear-gradient(135deg,#2c2200 0%,#4a3a00 50%,#2c2200 100%);border:3px solid #ffd700;border-radius:18px;padding:22px;margin:22px 0;box-shadow:0 0 25px rgba(255,215,0,0.7);text-align:center;animation:tp2-pulse 2.5s infinite;}
    ...
    .diagnosis {background:#1e2937;padding:15px;border-radius:12px;}
</style>
...
st.caption(f"最后更新: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} | K线: {len(candle_buffer)} | 高周期趋势: {'🟢' if higher_trend=='up' else '🔴' if higher_trend=='down' else '⚪'} {higher_trend.upper()}")
...
if len(candle_buffer) < 30:
    ...
    show_signal = signal
    if not show_signal and signal_history and signal_history and signal_history[0]['result'] == 'pending':
    ...
    if signal_history and signal_history[0]['result'] == 'pending':
        ...
        if rec['side'] == 'BUY' and cp > rec['peak']:
            ...
            updated = True
        if updated:
            update_signal_in_db(rec['id'], peak=rec['peak'])
    ...
    if show_signal:
        ...
        <div class="tp2-container">
            ...
            <div style="margin-top:8px;color:#a0ff9d;font-size:18px;">RR <strong>2:1</strong> 潜在盈利 +{profit_pts:.2f}点 (+{profit_pct:.1f}%)</div>
        </div>
        ''', unsafe_allow_html=True)
    ...
    with st.expander("🔍 当前市场诊断（为什么还没出信号）", expanded=True):
        if not df.empty:
            ...
            vol_ratio = last['volume'] / df['volume'].rolling(20).mean().iloc[-1] if df['volume'].rolling(20).mean().iloc[-1] > 0 else 0

            col_d1, col_d2, col_d3, col_d4 = st.columns(4)
            with col_d1: st.metric("EMA扩散强度", f"{ema_spread_pct:.3f}%", "✅ 达标" if ema_spread_pct >= 0.1 else "❌ 未达标")
            with col_d2: st.metric("EMA斜率", f"{slope:.5f}", "✅ 达标" if abs(slope) >= (0.00015 if use_aggressive else 0.00022) else "❌ 未达标")
            with col_d3: st.metric("20根波动率", f"{range_pct:.2f}%", "✅ 达标" if range_pct >= 0.3 else "❌ 横盘")
            with col_d4: st.metric("成交量爆发", f"{vol_ratio:.2f}x")
    ...
    with col1:
        ...
        fig.add_trace(go.Bar(x=plot_df.index, y=plot_df['volume'], name='成交量', marker_color=colors, opacity=0.85, showlegend=False), row=2, col=1)
...
st.markdown("---")
st.caption("🔥 极致版 v2026.02.26 • 成交量0智能回退 + 实时诊断 + 激进测试模式 + 全中文 + 风险仓位 • 直接复制运行即可！祝你今天就大赚！💰🚀")
