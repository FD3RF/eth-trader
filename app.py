...
def fetch_market_data(symbol, limit=100):
    ...
    try:
        ...
        klines = exchange.fetch_ohlcv(symbol=symbol, timeframe='5m', limit=limit)
...
def detect_volume_state(df):
    ...
    if current_vol < avg_vol * 0.6:
        ...
        return "巨量" if current_vol > avg_vol * 3 else "放量"
    else:
        return "正常量"
...
def detect_breakout(df, key_levels, use_close=False):
    current = df.iloc[-1]
    price_for_high = current['close'] if use_close else current['high']
    price_for_low = current['close'] if use_close else current['low']
    results = []
...
def detect_volume_trap(df):
    ...
    if body < atr * 0.3:
        ...
        if "放量" in vol_state or "巨量" in vol_state:
            return "放量滞涨（可能诱多）" if current['close'] > current['open'] else "放量滞跌（可能诱空）"
    return None
...
def generate_signals(df, trend, range_state, trap_signal, breakout, vol_state):
    long_reasons, short_reasons, score, warnings = [], [], 0, []
    
    # 做多条件
    if vol_state in ["缩量", "正常量"] and "回踩前低" in breakout:
        if "向上突破" in breakout:
            long_reasons.append("缩量回踩前低 + 向上突破 → 强做多信号"); score += 4
        else:
            long_reasons.append("缩量回踩前低（等待放量突破）"); score += 1
    if range_state == "低位横盘" and vol_state in ["缩量", "正常量"] and "向上突破" in breakout:
        long_reasons.append("低位缩量横盘 + 向上突破 → 平台突破做多"); score += 3
    if "向上突破" in breakout and vol_state in ["放量", "巨量"]:
        long_reasons.append("放量向上突破"); score += 2
    if trap_signal == "放量滞跌（可能诱空）" and "回踩前低" in breakout:
        long_reasons.append("放量暴跌不破 → 诱空陷阱，反向做多"); score += 3
    
    # 做空条件
    if vol_state in ["缩量", "正常量"] and "反弹前高" in breakout:
        if "向下突破" in breakout:
            short_reasons.append("缩量反弹前高 + 向下突破 → 强做空信号"); score += 4
        else:
            short_reasons.append("缩量反弹前高（等待放量跌破）"); score += 1
    if range_state == "高位横盘" and vol_state in ["缩量", "正常量"] and "向下突破" in breakout:
        short_reasons.append("高位缩量横盘 + 向下突破 → 平台突破做空"); score += 3
    if "向下突破" in breakout and vol_state in ["放量", "巨量"]:
        short_reasons.append("放量向下突破"); score += 2
    if trap_signal == "放量滞涨（可能诱多）" and "反弹前高" in breakout:
        short_reasons.append("放量暴涨不破 → 诱多陷阱，反向做空"); score += 3
    ...
    if trend == "上升趋势":
        ...
        final_signal, reasons = "📈 做多", long_reasons
    elif trend == "下降趋势":
        if not short_reasons:
            return "⏳ 观望（下降趋势无做空信号）", [], 0
        final_signal, reasons = "📉 做空", short_reasons
    else:  # 震荡
        if long_reasons and not short_reasons:
            ...
            final_signal, reasons = "🤔 观望（信号矛盾）", long_reasons + short_reasons
        else:
            final_signal, reasons = "⏳ 观望（无明确信号）", []
    
    strength = min(5, max(0, score))
    return final_signal, reasons + warnings, strength
...
if df is not None and len(df) > 20:
    ...
    signal, reasons, strength = generate_signals(df, trend, range_state, trap_signal, breakout, vol_state)
    current_price = df['close'].iloc[-1]
    ...
    with chart_placeholder.container():
        fig = go.Figure(data=[go.Candlestick(
            x=df.index[-30:], open=df['open'][-30:], high=df['high'][-30:],
            low=df['low'][-30:], close=df['close'][-30:], name='K线'
        )])
