...
def get_commander_intel(f_ema=12, s_ema=26, bar="15m"):
    try:
        ...
        # 【核心修复】必须在此注入字段，确保 DataFrame 返回即包含 ema_f
...
def main():
    ...
    with st.sidebar:
        ...
        # 【关键修复】映射 Plotly 标准主题名，彻底解决 ValueError
        ...
        st.info("🔍 AI 复盘：趋势量价匹配良好" if prob > 55 else "⚠️ AI 复盘：行情处于选择期，保持克制")

    # B. 主屏幕展示
    if df.empty:
        st.error("📡 卫星链路重组中..."); time.sleep(2); st.rerun()
    ...
    with col_chart:
        ...
        # 主图 K线         fig.add_trace(go.Candlestick(x=df['time'], open=df['o'], high=df['h'], low=df['l'], close=df['c'], name="K线"), row=1, col=1)
        ...
        fig.add_trace(go.Bar(x=df['time'], y=df['net_flow'], marker_color=colors, name="实时净流"), row=2, col=1)
