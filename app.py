...
# 配置参数
# ================================
st.set_page_config(layout="wide", page_title="ETH 100x Dual AI (OKX)")
...
def init_system():
    ...
    # 若没有独立模型，尝试加载通用模型（假设 predict_proba[:,1] 为多头）
    if model_long is None or model_short is None:
        ...
        if generic is not None:
            model_long = generic
            model_short = generic
            st.sidebar.info("⚠️ 使用通用模型镜像多空，建议训练专用双模型")
        else:
            st.sidebar.error("❌ 未找到任何模型文件！")
    
    return exch, model_long, model_short
...
# 主界面
# ================================
st.title("⚔️ ETH 100x 双核强化 AI 监控 (OKX)")
...
try:
    ...
    if st.session_state.system_halted:
        ...
        if model_long is not None and model_short is not None:
            prob_l = model_long.predict_proba(current_feat)[0][1]
            prob_s = model_short.predict_proba(current_feat)[0][1]
        else:
            prob_l = prob_s = 0.5

        # 极端概率警告
        if prob_l > 0.99 or prob_s > 0.99:
            st.warning("⚠️ 监测到极端置信度信号 (>99%)，请确认是否处于数据断层或剧烈波动期。")
        ...
        # 止损止盈建议（基于原始 ATR，放宽至 1.5 倍，上限 0.4%）
        atr = df['atr'].iloc[-1]
        sl_dist = min(atr * 1.5, current_price * 0.004)   # 止损距离（1.5倍ATR与0.4%取小）
        if side == "LONG":
