...
# ------------------- 页面配置 -------------------
st.set_page_config(
    page_title="5分钟量价交易系统（全自动）",
    page_icon="📊",
    layout="wide"
)

st.title("📈 5分钟合约全自动量价盯盘系统")
st.markdown("基于缩量/放量/横盘/突破口诀 + ATR动态风控 | 数据自动刷新")
...
with st.sidebar:
    ...
    # 突破确认方式
    confirm_with_close = st.checkbox("突破确认需收盘价在区间外", value=True, help="勾选后以收盘价判断突破，减少插针干扰")
...
# ------------------- 数据获取函数 -------------------
@st.cache_data(ttl=refresh_interval)  # 缓存时间等于刷新间隔
...
def detect_volume_state(df):
    ...
    avg_vol = df['vol_ma'].iloc[-2] if len(df) > 1 else current_vol
...
# ------------------- 信号生成函数 -------------------
def generate_signals(df, trend, range_state, trap_signal):
    """
    基于量价口诀生成交易信号
    返回: (信号类型, 理由列表, 强度分数)
    """
    ...
    # 根据趋势过滤（上升趋势只做多，下降趋势只做空，震荡多空都考虑）
    if trend == "上升趋势":
        if not long_reasons:
...
if df is not None and len(df) > 20:
    ...
    # 生成信号
    signal, reasons, strength = generate_signals(df, trend, range_state, trap_signal)
    ...
    with metrics_placeholder.container():
        ...
        cols[1].metric("ATR(14)", f"{current_atr:.2f}")
    ...
    with signal_placeholder.container():
        ...
        # 信号强度
        if signal not in ["🤔 观望（信号矛盾）", "⏳ 观望（无明确信号）", "⏳ 观望（上升趋势无做多信号）", "⏳ 观望（下降趋势无做空信号）"]:
        ...
        if "放量滞涨" in trap_signal:
            tips.append("📌 放量滞涨，主力可能出货，避免追多")
        if "放量滞跌" in trap_signal:
