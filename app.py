...
if enable_scan and run_scan:
    ...
    for idx, (b_th, v_period, br_th) in enumerate(param_combinations):
        ...
        # 构建特征（注意：特征构建依赖于 lookback 和 vol_ma_period，此处 vol_ma_period 使用扫描值）
        df_feat_scan = build_features(df, lookback, v_period)
        if len(df_feat_scan) == 0:
            continue
        if enable_oos:
            split_point = int(len(df_feat_scan) * train_ratio)
            train_df = df_feat_scan.iloc[:split_point]
            test_df = df_feat_scan.iloc[split_point:]
            # 只记录测试集结果
            _, test_equity = simulate(test_df, 0, len(test_df),
                                      fee_rate, slippage, apply_costs,
                                      rr_ratio, min_hold, br_th, b_th)
            # 为了简化，我们只记录测试集交易数和关键指标（可自定义）
            # 这里为了速度，仅模拟交易次数较少的测试集，并提取统计
            # 但为了准确，我们应该完整模拟并获取records，但那样太慢
            # 此处我们临时修改：直接调用 simulate 返回 records 和 equity，然后 compute_stats
            test_records, test_equity = simulate(test_df, 0, len(test_df),
                                                 fee_rate, slippage, apply_costs,
                                                 rr_ratio, min_hold, br_th, b_th)
            ...
            # 全样本扫描
            df_feat_scan = build_features(df, lookback, v_period)
            records, equity = simulate(df_feat_scan, 0, len(df_feat_scan),
                                        fee_rate, slippage, apply_costs,
                                        rr_ratio, min_hold, br_th, b_th)
