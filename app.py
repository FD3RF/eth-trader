...
# ========================== 历史记录（列安全初始化） ==========================
def load_history():
    cols = ["time", "direction", "entry", "stop", "tp", "result", "score", "rr", "quality", "net", "signal_ts"]
    if os.path.exists(HISTORY_FILE):
        dfh = pd.read_csv(HISTORY_FILE)
        for c in cols:
            if c not in dfh.columns:
                dfh[c] = None if c != "score" and c != "rr" and c != "net" else 0
        return dfh[cols]
...
st.subheader("📜 最近信号")
st.dataframe(history.tail(15)[["time", "direction", "entry", "stop", "tp", "result", "quality", "rr"]], width='stretch')
...
st.caption("✅ **云端终极完美版** 已运行 | 无任何警告 | 纯模拟监控 | 100元高频极易爆仓，实盘前必须OKX模拟盘测试30天！")
