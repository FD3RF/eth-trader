...
# 0. 指挥部核心配置 (全宽屏模式)
# ==========================================
st.set_page_config(layout="wide", page_title="ETH V32026 永恒稳定版", page_icon="⚖️")
...
def get_commander_intel(f_ema, s_ema, bar="15m"):
    try:
        ...
        # B. 【指标原子计算】
        # EMA 计算公式：EMA = (Price - Prev_EMA) * Multiplier + Prev_EMA
...
def main():
    ...
    with st.sidebar:
        ...
        st.markdown(f"""
            <div style="border:2px solid {status_color}; padding:20px; border-radius:15px; text-align:center; background:rgba(0,0,0,0.6); box-shadow: 0 0 20px {status_color}44;">
                ...
                <strong style="color:{status_color}; font-size:2.8em;">{prob:.1f}%</strong><br>
                <small style="color:#666;">策略盈亏比校准: 1 : 1.85</small>
            </div>
        """, unsafe_allow_html=True)

    # --- 主显示区：状态校验屏障 ---
    if df.empty or 'ema_f' not in df.columns:
        st.warning("📡 卫星链路同步中... 正在校准情报源"); time.sleep(2); st.rerun()
    ...
    with col_strat:
        st.markdown("#### 🎯 实时战术指令")
        st.success(f"✅ **多头激活 | 陷阱捕获**\n\n目标位: ${last_p + atr*1.85:.1f}\n防守位: ${last_p - atr:.1f}")
        st.warning(f"🔥 **进攻提示 | 清算猎杀**\n\nEMA 偏离度: {(abs(last_p - df['ema_f'].iloc[-1])/last_p)*100:.2f}%")
        st.error(f"🚨 **防守预警 | 量价共振**\n\n核心支撑: ${df['ema_s'].iloc[-1]:.1f}")
