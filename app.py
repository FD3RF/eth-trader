# -*- coding: utf-8 -*-
"""
🚀 终极量化终端 · 职业版 48.3 (终极融合版 · 生产就绪)
===================================================
[版本说明]
- 融合 48.1 与 48.2 所有优势
- 资金费精确对齐 Binance UTC 0/8/16 结算点
- 协方差矩阵奇异/NaN 时回退等风险贡献
- WebSocket 订单监听异常自动降级到 REST 轮询
- ML 训练彻底杜绝未来信息泄露
- SQLite WAL 模式 + 全局连接池（线程安全）
- API 密钥通过环境变量管理（安全）
- 因子权重每小时定时更新
- 回测带进度条
- 代码结构优化、注释完善、冗余清理
...
def check_dependencies() -> None:
    required_packages = [
        'streamlit', 'pandas', 'numpy', 'ta', 'ccxt', 'requests',
        'plotly', 'scipy', 'pytz', 'sklearn', 'joblib', 'skopt',
...
class TradingConfig:
    ...
    training_schedule_interval: int = 3600
    signal_cache_ttl: int = 60
    funding_fee_hours: int = 8
